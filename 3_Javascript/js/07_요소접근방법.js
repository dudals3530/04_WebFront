...
function classTest() {
  // 유사배열이란 ?
  // 배열처럼 index, length를 가지고있으나,
  // 배열전용 기능 (메서드)를 제공하지 않음.
  // 제공하지 않음 ;pop(), push(), map(), filter() ...
  ...
  // JS에서도 .style을 이용해서 css 스타일을 만져줄수잇음
...
function nameTest() {
  ...
  for (let i = 0; i < hobbyList.length; i++) {
    ...
    // input요소,checked -> 요소가 체크되어있다면 true,아니면 flase
    // input요소.checked = true;  -> 해당요소를 체크하겠다.
    // input요소.checked = false; -> 해당요소를 체크 해제 하겠다.
    ...
    // #name-div  요소에 내용으로 결과를 출력해보자.
    document.getElementById(
      "name-div"
    ).innerHTML = `${str} <br><br>선택된 취미개수 : ${count}`;
  }
...
function cssTest() {
  ...
  div2.innerText = "첫번째 요소가 아니면 querySelector() 특징 확용 못함";
  ...
  //인텍스를 이용해서 요소 하나씩 접근
  divList[0].style.fontFamily = "궁서체"; // 난 궁서체 까지써야 바뀌니ㅔㅇㅇ?
...
/** 
 아이디가 user-input 인 input 요소에서 
 키가 올라오는 동작이 발생했을떄 (감지 되었을때)
 올라온키가 "Enter"키 이면 readValue()함수 호출
 */

// keydown: 키가 눌러졌을떄 (+ 꾹 누르고있으면 계속 인식됨.)
// keyup : 눌러지던 키가 떼어졌을떄 (1회만 인식함.)
